...
@php
    ...
    $badgeClasses = Flux::classes()
        ...
        ->add($badgeVariant === 'outline' ? ['after:absolute after:inset-[3px] after:bg-white dark:after:bg-zinc-900', $badgeCircle ? 'after:rounded-full' : 'after:rounded-[1px]'] : [])
        ->add(
            match ($badgePosition) {
...
<flux:with-tooltip :$tooltip :$attributes>
    ...
    >
        ...
        <img
            src="{{ $src }}"
            alt="{{ $alt ?? $name }}"
            class="size-full rounded-[var(--avatar-radius)] object-cover"
        />
        ...
        <div
            {{ $badge->attributes->class($badgeClasses) }}
            aria-hidden="true"
        >
            {{ $badge }}
        </div>
